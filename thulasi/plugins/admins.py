...
@thulasi.on(events.callbackquery.CallbackQuery(data="admin"))
async def _(event):

    await event.edit(ADMIN_TEXT, buttons=[[Button.inline("« Bᴀᴄᴋ", data="help")]])

@thulasi.on(events.callbackquery.CallbackQuery(data="play"))
async def _(event):

    await event.edit(PLAY_TEXT, buttons=[[Button.inline("« Bᴀᴄᴋ", data="help")]])

@thulasi.on(events.NewMessage(pattern="^[!?/]promote ?(.*)"))
@is_admin
async def promote(event, perm):
    ...
    if not input_str and not user:
        ...
        return
    sed = await thulasi(GetFullUserRequest(id=user.sender_id or input_str))
...
@thulasi.on(events.NewMessage(pattern="^[!?/]demote ?(.*)"))
@is_admin
async def promote(event, perm):
    if Config.MANAGEMENT_MODE == "ENABLE":
        return
...
@thulasi.on(events.NewMessage(pattern="^[!?/]invitelink"))
async def invitelink(event):
    ...
    if event.is_private:
       await event.reply("This cmd is made to be used in groups, not in PM!")
       return
    link = await thulasi(ExportChatInviteRequest(event.chat_id))
