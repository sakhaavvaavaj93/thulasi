...
from thulasi import call_py, thulasi, client as Client
owner = "1669178360"
from thulasi.helpers.yt_dlp import bash
from thulasi.helpers.chattitle import CHAT_TITLE
from thulasi.helpers.queues import (
    QUEUE,
    add_to_queue,
    clear_queue,
    get_queue,
...
from thulasi.helpers.thumbnail import gen_thumb
from thulasi.helpers.joiner import AssistantAdd
...
@thulasi.on(events.callbackquery.CallbackQuery(data="cls"))
async def _(event):

     await event.delete()

#play
@thulasi.on(events.NewMessage(pattern="^[?!/]play"))
@AssistantAdd
async def play(event):
    ...
    if (
        ...
        if search == 0:
            ...
            format = "best[height<=?320][width<=?640]"
            hm, ytlink = await ytdl(format, url)
            if hm == 0:
                await botman.edit(f"`{ytlink}`")
            elif chat_id in QUEUE:
                pos = add_to_queue(chat_id, songname, ytlink, url, "Audio", 0)
                caption = f"**playing..on.Vc**"
                await botman.delete()
        ...
        if chat_id in QUEUE:
            ...
            caption = fcaption = f"**playing..on.Vc**"
            await event.client.send_file(chat_id,caption=caption)
            await botman.delete()
        else:
            try:
...
#end
@thulasi.on(events.NewMessage(pattern="^[/?!]end"))
@is_admin
...
@thulasi.on(events.NewMessage(pattern="^[?!/]vplay"))
@AssistantAdd
    ...
    if replied and not replied.video and not replied.document:
        ...
        if search == 0:
            ...
            if hm == 0:
                ...
                caption = f"💡 **Video Streaming Queue »** `#{pos}`"
                await event.client.send_file(chat_id, caption=caption)
            else:
                try:
                    await call_py.join_group_call(
...
#playlist
@thulasi.on(events.NewMessage(pattern="^[?!/]pl,q"))
@is_admin
...
#leavevc
@thulasi.on(events.NewMessage(pattern="^[?!/]leavevc,leave"))
@is_admin
...
@thulasi.on(events.NewMessage(pattern="^[?!/]skip"))
@is_admin
...
@thulasi.on(events.NewMessage(pattern="^[?!/]pause"))
@is_admin
...
@thulasi.on(events.NewMessage(pattern="^[?!/]resume"))
@is_admin
